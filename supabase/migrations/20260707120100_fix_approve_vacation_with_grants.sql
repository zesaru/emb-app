-- Fixes two bugs in public.approve_vacation_with_grants (202603260005):
-- 1. A trailing comma before `where` made both UPDATE statements invalid SQL,
--    breaking vacation approval in both the grant-balance and legacy-fallback paths.
-- 2. The function is SECURITY DEFINER but never verified the caller was an
--    active admin, nor that p_approved_by matched the caller — any authenticated
--    user could call it directly and force-approve any vacation.
...
begin
  ...
  if p_approved_by <> auth.uid() then
    raise exception 'APPROVER_MISMATCH';
  end if;

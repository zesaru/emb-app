...
describe("superAdminForceCancelCompensatorio", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("rechaza si el usuario autenticado no es super admin", async () => {
    ...
    const superAdminForceCancelCompensatorio = (
      await import("@/actions/super-admin-force-cancel-compensatorio")
    ).default;
    const result = await superAdminForceCancelCompensatorio(validCompensatorioId);
  ...
  it("rechaza un id inválido sin llamar al RPC", async () => {
    ...
    const result = await superAdminForceCancelCompensatorio("not-a-uuid");
  ...
  it("elimina correctamente vía RPC cuando el usuario es super admin, incluso ya aprobada", async () => {
    ...
    expect(rpcMock).toHaveBeenCalledWith("super_admin_force_cancel_compensatorio", {
